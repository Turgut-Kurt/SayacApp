        ...
        return (
            <TouchableHighlight onPress={() => testPeripheral(item)}>
                <View style={[styles.row, { backgroundColor: "white" }]}>
                    <Text style={{ fontSize: 12, textAlign: 'center', color: color, padding: 10 }}>{item.name}</Text>
                    <Text style={{ fontSize: 8, textAlign: 'center', color: color, padding: 2, paddingBottom: 20 }}>{item.id}</Text>
                </View>
            </TouchableHighlight>
        );
    }
        ...
        <View style={styles.Container}>
            ...
            <View style={styles.textStyle}>
                ...
                />) : (found ? (<FlatList
                    ...
                    renderItem={({ item }) => renderItem(item)}
                    keyExtractor={item => item.id}
                />) : (<Text style={{ textAlign: 'center', color: "red" }}>{isScanning ? "Scanning" : "No device found"}</Text>)
                )}
            </View>
